...
function Servers() {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/admin/servers", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        setMessage(data.message || "Server details save failed");
        return;
      }

      setMessage("Server details saved successfully");
    } catch (error) {
      console.log(error);
    ...
    >
      <style>
        {`
          ...
          .servers-scroll::-webkit-scrollbar-track {
            background: transparent;
          }

          .servers-scroll::-webkit-scrollbar-thumb {
            background: rgba(0, 184, 148, 0.45);
            border-radius: 999px;
          }

          .servers-scroll::-webkit-scrollbar-thumb:hover {
      ...
      <div className="mb-5 flex shrink-0 items-center justify-between gap-6">
        <div className="relative w-full max-w-[430px]">
          ...
          <input
            type="text"
            placeholder="Search server and security details..."
      ...
      <div className="mb-5 flex shrink-0 items-center justify-between gap-5">
        <div>
          ...
          >
            Servers, Applications & Security
          </h1>
          <p className={`mt-1.5 text-base ${mutedTextClass}`}>
            Provide on-premise infrastructure, application, backup, endpoint,
        ...
        <div className="flex h-full min-h-0 flex-col">
          <div className={`mb-6 shrink-0 border-b pb-6 ${dividerClass}`}>
            ...
            <p className={`mt-2 text-base ${mutedTextClass}`}>
              Enter server, application, backup, endpoint protection, firewall,
              and incident response information.
            </p>
          </div>

          {/* ONLY FORM CONTENT SCROLLS */}
          <div className="servers-scroll min-h-0 flex-1 overflow-y-auto pr-2">
            <div className="grid grid-cols-1 gap-x-8 gap-y-7 xl:grid-cols-2">
              <FormInput
                label="On-Prem Server"
                name="onPremServer"
                value={formData.onPremServer}
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Enter on-prem server details"
                required
              ...
              <FormInput
                ...
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Enter server purpose"
                required
                isLightTheme={isLightTheme}
              />
              ...
              <FormInput
                ...
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: VMware, Hyper-V, Proxmox"
                isLightTheme={isLightTheme}
              />
              ...
              <FormInput
                ...
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Enter critical applications"
                isLightTheme={isLightTheme}
              />
              ...
              <FormInput
                ...
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: Veeam, Azure Backup, Acronis"
                isLightTheme={isLightTheme}
              />
              ...
              <FormInput
                ...
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: 30 days, 7 years, RPO/RTO details"
                isLightTheme={isLightTheme}
              />

              <FormInput
                label="Endpoint Protection / EDR Vendor"
                name="endpointProtectionEdrVendor"
                value={formData.endpointProtectionEdrVendor}
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: Microsoft Defender, CrowdStrike"
                isLightTheme={isLightTheme}
              />
              ...
              <FormInput
                ...
                value={formData.mobileDeviceManagement}
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: Intune, Jamf, Workspace ONE"
                isLightTheme={isLightTheme}
              />

              <FormInput
                label="Patch Management Tool"
                name="patchManagementTool"
                value={formData.patchManagementTool}
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: Intune, SCCM, WSUS"
                isLightTheme={isLightTheme}
              />

              <FormInput
                label="Firewall Vendor"
                name="firewallVendor"
                value={formData.firewallVendor}
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: Fortinet, Palo Alto, Cisco"
                isLightTheme={isLightTheme}
              />

              <FormInput
                label="Email Filtering / DLP"
                name="emailFilteringDlp"
                value={formData.emailFilteringDlp}
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Example: Microsoft Defender, Proofpoint"
                isLightTheme={isLightTheme}
              />
              ...
              <FormInput
                ...
                onChange={handleChange}
                disabled={!canEdit}
                placeholder="Enter incident response plan status"
                isLightTheme={isLightTheme}
              />
            </div>
          </div>
          ...
          >
            <div>
              ...
              >
                These details help us assess application dependencies, backup
                readiness, endpoint protection, patching, firewall posture, and
